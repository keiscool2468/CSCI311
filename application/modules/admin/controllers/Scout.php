...
class Scout extends Admin_Controller {
	...
	public function my_personal_particular()
	{
		// print_r($this->mUser->user_id);exit;
		$user_id = $this->mUser->user_id;
		$checkPP = $this->db->get_where('scout_personal_particulars', array('user_id' => $user_id));
	   	if($checkPP->num_rows()==0){
	   		$this->render('errors/error_pp_empty');
	   	} 
	   	else{
	   		$this->mPageTitle = 'Personal Particular';
		// print_r($record);
	   		$thisPP = $checkPP->result_array();
			$region = [
			'Kowloon',
			 'East Kowloon',
			  'New Territories',
			...
			$this->render('scout/my_personal_particular');
	  	}
	...
	{	
		$this->mPageTitle = 'My Progressive Award';
		$this->render('scout/my_award_list');
	}
	...
	public function myMembership()
	{
		...
		$thisRecord = $this->db->get_where('scout_award_update_records', array('user_id' => $user_id, 'award_id' => 1, 'sign' => 1, 'leatest' => 1))->result_array();
	   	if($award->num_rows() == 0) {
	   		$this->render('errors/error_award_empty');
	   	} else { 
			...
			$this->render('scout/my_membership_award');
		}
	}

			// $this->render('errors/error_award_empty');

	public function myPathfinder()
	{
		$user_id = $this->mUser->user_id;
		$sign = array(
			'A1a'		=>	'',
		...
		if($award->num_rows() == 0) {
			...
			$awardPath = $this->db->get_where('scout_personal_particulars', array('user_id' => $user_id))->result_array()[0]['award_path'];
			$award_detail = array(
			...
			$this->render('scout/my_pathfinder_award');
		}
	}

	public function myStandard()
	{	
		$user_id = $this->mUser->user_id;
		$sign = array(	  		
			'A1a'		=>	'',
		...
		$thisRecord = $this->db->get_where('scout_award_update_records', array('user_id' => $user_id, 'award_id' => 3, 'sign' => 1, 'leatest' => 1))->result_array();
	   	if($award->num_rows()==0) {
			...
			$this->render('scout/my_standard_award');
		}
	}

	public function myadvanced()
	{
		$user_id = $this->mUser->user_id;
		$sign = array(	  		
			'A1a'		=>	'',
		...
		$thisRecord = $this->db->get_where('scout_award_update_records', array('user_id' => $user_id, 'award_id' => 4, 'sign' => 1, 'leatest' => 1))->result_array();
	   	if($award->num_rows() == 0) {
			...
			$this->render('scout/my_advanced_award');
	   	}
	}

	public function myChief()
	{
		$user_id = $this->mUser->user_id;
		$sign = array(	  		
			'A1a'		=>	'',
		...
		$thisRecord = $this->db->get_where('scout_award_update_records', array('user_id' => $user_id, 'award_id' => 5, 'sign' => 1, 'leatest' => 1))->result_array();
	   	if($award->num_rows()==0) {
			...
			$this->render('scout/my_chief_award');
	   	}
	}
}